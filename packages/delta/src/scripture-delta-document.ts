...
export class ScriptureDeltaDocument extends DeltaDocument implements ScriptureDocument {
  ...
  update(changes: readonly Op[], version: number): void {
    const change = new Delta(changes as Op[]);
    const [changeStartOffset, changeEndOffset, insertLength] = getChangeOffsetRange(change);
    ...
    const updated = this._content.compose(change);
