...
export class ScriptureDeltaDocument extends ScriptureDocumentMixin(DeltaDocument) {
  ...
  update(changes: Op[] | Delta, version: number): void {
    ...
    const childStartLine =
      childStartIndex < this.children.length ? this.children[childStartIndex].range.start.line : lineChildren.length - 1;
    const childStartPosition = childStart?.range.start ?? this.children[this.children.length - 1].range.end;
    const childEndLine =
