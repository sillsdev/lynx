...
export class ExamplePunctuationChecker extends AbstractChecker {
  ...
  protected getFixes(textDocument: TextDocument, diagnostic: Diagnostic): DiagnosticFix[] {
    if (diagnostic.code === 1) {
      return [
        StandardFixes.punctuationRemovalFix(textDocument, diagnostic)
      ];
    }
    return [];
  }
...
class QuotationErrorFinder {
  ...
  private handleUnmatchedQuotationMarks(): void {
    while(this.quoteStack.length > 0) {
      const quotationMark = this.quoteStack.pop();

      this.addUnmatchedOpeningQuoteError(quotationMark as QuoteMetadata);
    }
  }

  private addNestedQuoteError(quotationMark: QuoteMetadata): void {
    const diagnostic: Diagnostic = this.diagnosticFactory.newBuilder()
      .setCode(1)
