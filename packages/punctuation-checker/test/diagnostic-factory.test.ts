...
import { Diagnostic, DiagnosticSeverity } from 'lynx-core';

it('correctly builds Diagnostic objects', () => {
  const diagnosticFactory: DiagnosticFactory = new DiagnosticFactory('test-source-id', new MockSingleLineTextDocument(""));

  const code = 'testing-code';
  const message = 'testing-message';

  const diagnostic: Diagnostic = diagnosticFactory.newBuilder()
    .setCode(code)
    .setMessage(message)
    .setRange(5, 10)
...
it('produces errors when the Diagnostic object is incompletely specified', () => {
  ...
  const diagnosticMissingCode: () => Diagnostic = () => {
    return diagnosticFactory.newBuilder()
      .setMessage(message)
      .setRange(5, 10)
      .setSeverity(DiagnosticSeverity.Warning)
      .build();
